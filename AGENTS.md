...
**本地优先的记忆系统**：记忆文件存储在 Codex 原生 memory 目录，可选用云同步工具（OneDrive/Dropbox）实现跨设备。

- **主存储目录**：`C:\Users\fjyu9\.Codex\projects\C--Users-fjyu9\memory\`
...
1. **设计文档**: `.Codex/2026-06-05-ai-task-flow-design.md`
   - 完整的产品定位、架构设计、技术选型
   - 必读，理解整体方向

2. **实施计划**: `.Codex/2026-06-05-ai-task-flow-implementation-plan.md`
...
```bash
# 在项目根目录
cat .Codex/2026-06-05-ai-task-flow-design.md
```
...
打开实施计划：
```bash
cat .Codex/2026-06-05-ai-task-flow-implementation-plan.md
```
...
| 技术 | 理由 |
|------|------|
| MCP Server | 保留原生 Codex 体验，官方协议长期稳定 |
...
**Q: 为什么不用 Agent SDK 嵌入 Codex？**
A: v2 设计尝试过，但用户反馈"失去原生 Codex 体验"。v3 通过 MCP 协议让用户完全掌控 AI 交互。
...
- **10% E2E 测试**: 完整流程（录入 → 派发 → Codex 拉取 → 回写 → 审查）
